...
public class ListGenerator {
	...
	public DoubleList<String, Widget> listViews;

	private ListGenerator() {
		listViewNames = new ArrayList<String>();
		listViews = new DoubleList<String, Widget>();
	}
	...
	private boolean isFeatureAnAssignment(EStructuralFeature feature) {

		String layoutId = feature.getEAnnotations().get(0).getDetails().get("layoutId");
		return layoutId != null;

	}

	public void createList(EPackage screenPackage) {

		for (EClassifier classifier : screenPackage.getEClassifiers()) {
			...
			for (EStructuralFeature feature : eClass.getEStructuralFeatures()) {
				...
				if (isFeatureAnAssignment(feature)) {	//
					String viewComponentName = feature.getEAnnotations().get(0).getDetails().get("useComponent");
					if (feature.isMany()) {
						if (viewComponentName == null) {
							String listViewId = feature.getName() + LISTVIEW + counter;
							counter++;
							handleSimpleList(listViewId);
							System.out.println("###start2###");
							System.out.println("classname: " + eClass.getName());
							System.out.println("feature: " + feature.getName());
							System.out.println("Creating listView and its files for id: " + listViewId);
						...
						//Else, check whether the assignment uses a view component
					}
				}
...
//	public void createLists(EPackage thePackage) {
//
//		for (EStructuralFeature feature : populatedInstance.eClass().getEStructuralFeatures()) {
//
//			if (isFeatureAnAssignment(feature)) {	//
//				String layoutId = feature.getEAnnotations().get(0).getDetails().get("ocl");
//				String viewComponentName = feature.getEAnnotations().get(0).getDetails().get("useComponent");
//				if (feature.isMany()) {
//					if (viewComponentName == null) {	//plain string list
//						handleSimpleList("listView");
//					} else {	//complex list
//
//					}
//				}	//Else, check whether the assignment uses a view component
...
//	}
//
	private void handleSimpleList(String listViewName) {
